...
class Hangman:
    ...
    def game_state(self):
        ...
        if len(self.letters_tried) > 0:
            print(">>> Stav hry:\n"
                  "Uz si zkousel tato pismena:", *self.letters_tried)
        else:
            print("##### Zaciname! #####")
        
        print(f"Zbyva ti {self.tries_left}/{self.tries} pokusu!")
    ...
    def is_over(self):
        ...
        for l in self.word:
            if l not in self.letters_tried:
                break
        else:
    ...
    def determine_win_or_defeat(self):
        print("Konec hry")
        print(f"Slovo bylo: {self.word}")
        all_letters_found = False
        for l in self.word:
            if l not in self.letters_tried:
                break
